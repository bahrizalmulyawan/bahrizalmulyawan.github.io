...
let dom = {
    ...
    compileAll: null as HTMLButtonElement | null,
    downloadZip: null as HTMLButtonElement | null,
    compileStatus: null as HTMLElement | null,
    compileDetail: null as HTMLElement | null,
    compileProgress: null as HTMLProgressElement | null,
};
...
/** 
 * Static-site mode: always decode the bundled Voyager MP3 asset.
 */
function getAudio(): void {
    const VOYAGER_AUDIO_URL = './src/assets/audio/voyager.mp3';
    fetch(VOYAGER_AUDIO_URL, { cache: 'no-store' })
        ...
        .then(decodedAudio => {
            ...
            dom.compileAll!.disabled = false;
            dom.compileStatus!.innerText = '[ AUDIO READY / 116 LOGICAL IMAGES ]';
            setTimeout(() => {
        ...
        .catch(error => {
            console.error('Error fetching or decoding bundled Voyager audio: ', error);
            if (dom.loader) dom.loader.innerHTML = '<p>[ ERROR ] AUDIO DECODER FAILED</p><p class="loader-sub">Could not load bundled src/assets/audio/voyager.mp3.</p>';
        });
}
...
interface logicalImageGroup {
    startIndex: number;
    passCount: number;
    kind: "bnw" | "rgb";
}

function getLogicalImageGroups(channel: channelObj): Array<logicalImageGroup> {
    const groups: Array<logicalImageGroup> = [];
    let i = 0;
...
function findNextPeakPosition(channel: channelObj, position: number): number {
...
function yieldRasterStep(): Promise<void> {
    return new Promise(resolve => setTimeout(resolve, RASTER_RENDER_DELAY_MS));
}

function drawRasterColumnToImageData(
    channel: channelObj,
    position: number,
    colIndex: number,
    rgb: string,
...
async function renderLogicalImage(
    channel: channelObj,
    group: logicalImageGroup,
    progressCallback: (columnsDone: number, totalColumns: number) => void,
): Promise<ImageData> {
...
function formatEncodedTime(samples: number): string {
...
function zipPartsConcat(parts: Uint8Array[]): Blob {
...
function setupImageCompiler(): void {
