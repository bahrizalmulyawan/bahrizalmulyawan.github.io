...
function setupVisiblePlayer(selected) {
    ...
    if (buffer) {
        ...
        audio.src = new URL('./assets/audio/voyager.mp3', document.baseURI).href;
...
function getAudio() {
    ...
    loadVoyagerAudio().then(arrayBuffer => audioContext.decodeAudioData(arrayBuffer))
      .then(decodedAudio => {
      ...
      .catch(error => {
        dom.loader.querySelector('.loader-sub').textContent='[ ERROR ] Could not load bundled Voyager audio from assets/audio/voyager.mp3.';
